...
func manualTest() {
fmt.Println("Running manual configuration tests...")
...
config1, err := LoadConfig("/tmp/single-queue.yaml")
if err != nil {
fmt.Printf("ERROR: Failed to load single queue config: %v\n", err)
os.Exit(1)
}

fmt.Println("✓ Test 1: Single queue mapping")
...
config2, err := LoadConfig("/tmp/multi-queue.yaml")
if err != nil {
fmt.Printf("ERROR: Failed to load multi queue config: %v\n", err)
os.Exit(1)
}
...
config3, err := LoadConfig("/tmp/mixed.yaml")
if err != nil {
fmt.Printf("ERROR: Failed to load mixed config: %v\n", err)
os.Exit(1)
}

fmt.Println("\n✓ Test 3: Mixed mapping (backward compatibility)")
